...
public class GatherUnusedDependenciesOperationExtensionsTest {
	...
	@Test
	public void testDependencyOnlyReferencedByTypeAttributeDeclaredInSchemaIsNotFlaggedAsUnused() throws Exception {
		String bundleProvider = "provider.schemaattribute";
		String packageProvider = createProviderBundle(bundleProvider);
		IProject projectConsumer = createConsumerBundleWithSchema("consumer.schemaattribute", bundleProvider, """
				<client class="%s.Sample"/>
				""".formatted(packageProvider));

		buildProjects();
		List<String> unusedPlugins = gatherUnusedDependencies(projectConsumer);
		assertFalse("Dependency providing a type referenced by an attribute that the schema declares to be of the kind "
				+ "java must not be flagged as unused", unusedPlugins.contains(bundleProvider));
	}

	@Test
	public void testDependencyOnlyReferencedByTypeElementDeclaredInSchemaIsNotFlaggedAsUnused() throws Exception {
		String bundleProvider = "provider.schemaelement";
		String packageProvider = createProviderBundle(bundleProvider);
		IProject projectConsumer = createConsumerBundleWithSchema("consumer.schemaelement", bundleProvider, """
				<client>
				         <class class="%s.Sample">
				            <parameter name="key" value="value"/>
				         </class>
		...
		buildProjects();
		List<String> unusedPlugins = gatherUnusedDependencies(projectConsumer);
		assertFalse("Dependency providing a type referenced by a type attribute given as nested element must not be "
				+ "flagged as unused", unusedPlugins.contains(bundleProvider));
	}

	@Test
	public void testDependencyOnlyReferencedByPlainAttributeDeclaredInSchemaIsFlaggedAsUnused() throws Exception {
		String bundleProvider = "provider.schemaplain";
		String packageProvider = createProviderBundle(bundleProvider);
		IProject projectConsumer = createConsumerBundleWithSchema("consumer.schemaplain", bundleProvider, """
				<client label="%s.Sample"/>
				""".formatted(packageProvider));

		buildProjects();
		List<String> unusedPlugins = gatherUnusedDependencies(projectConsumer);
		assertTrue("Dependency must be flagged as unused although a value has the shape of a type name, because the "
				+ "schema declares the attribute holding it to be a plain string", unusedPlugins.contains(bundleProvider));
	}
	...
	@Test
	public void testDependencyOnlyReferencedByUnresolvedTypeDeclaredInSchemaIsNotFlaggedAsUnused() throws Exception {
		String bundleProvider = "provider.schemanotype";
		String packageProvider = createProviderBundle(bundleProvider);
		IProject projectConsumer = createConsumerBundleWithSchema("consumer.schemanotype", bundleProvider, """
				<client class="%s.NoSuchType"/>
				""".formatted(packageProvider));
		...
		List<String> unusedPlugins = gatherUnusedDependencies(projectConsumer);
		assertFalse(
				"Dependency must not be flagged as unused although the referenced type does not resolve, because the "
						+ "schema declares the value to be a type",
				unusedPlugins.contains(bundleProvider));
	}
	 ...
	 * referenced from the contributed extension. No schema is provided for the
	 * extension point, so that the values are analyzed by their shape.
	 */
	...
	/**
	 * Creates a plug-in project that requires the given provider bundle and
	 * contributes the given element to an extension point declared by itself, for
	 * which it provides a schema that declares the {@code class} attribute of the
	 * contributed element to be of the kind {@code java} and its {@code label}
	 * attribute to be a plain string. The project has to be a singleton, as only
	 * then its extension point and thus the schema is resolvable.
	 */
	private static IProject createConsumerBundleWithSchema(String symbolicName, String providerSymbolicName,
			String extensionContent) throws Exception {
		IProject project = createJavaPluginProject(symbolicName, null, true);
		addRequiredBundle(project, providerSymbolicName);
		createFile(project, IPath.fromOSString("schema/clients.exsd"), """
				<?xml version='1.0' encoding='UTF-8'?>
				<schema targetNamespace="%1$s" xmlns="http://www.w3.org/2001/XMLSchema">
				   <annotation>
				      <appinfo>
				         <meta.schema plugin="%1$s" id="clients" name="Clients"/>
				      </appinfo>
				   </annotation>
		...
		createExtensionFile(project, "plugin", "plugin.xml", symbolicName, "schema/clients.exsd", extensionContent);
		return project;
	}
	...
	private static void createExtensionFile(IProject project, String rootElement, String fileName,
		...
		createExtensionFile(project, rootElement, fileName, declaringSymbolicName, null, extensionContent);
	}

	private static void createExtensionFile(IProject project, String rootElement, String fileName,
			String declaringSymbolicName, String schemaFile, String extensionContent) throws CoreException {
		String extensionPointId = "clients";
		String schemaReference = schemaFile != null ? " schema=\"" + schemaFile + "\"" : "";
		createFile(project, IPath.fromOSString(fileName), """
				<?xml version="1.0" encoding="UTF-8"?>
				<?eclipse version="3.4"?>
				<%1$s>
				   <extension-point id="%3$s" name="Clients"%5$s/>
